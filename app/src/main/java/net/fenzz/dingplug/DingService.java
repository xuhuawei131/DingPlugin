...
//       System.out.println("nodeInfo"+nodeInfo);
        ...
        switch (index) {
            ...
            case 2: //进入签到页
                ...
                OpenQianDao(event.getEventType(), nodeInfo1);
                break;
            case 3:
...
//			List<AccessibilityNodeInfo> homeList = nodeInfo.findAccessibilityNodeInfosByText("工作");
            List<AccessibilityNodeInfo> homeList = nodeInfo.findAccessibilityNodeInfosByViewId("com.alibaba.android.rimet:id/home_bottom_tab_text");
            if (!homeList.isEmpty()) {
                //点击
                for (AccessibilityNodeInfo item : homeList) {
                    String itemName = item.getText().toString();
                    Log.v("xhw", "itemName " + itemName);
                    if (itemName.equals("工作")){
                         onclick(item);  //遍历点击
                        index = 2;
...
//            System.out.println("点击进入签到页面详情");
            ...
            //判断当前是否是主页的签到页
            List<AccessibilityNodeInfo> qianList = nodeInfo.findAccessibilityNodeInfosByViewId("com.alibaba.android.rimet:id/home_bottom_tab_icon_large");
//            List<AccessibilityNodeInfo> qianList = nodeInfo.findAccessibilityNodeInfosByText("考勤打卡");
            if (!qianList.isEmpty()) {
                ...
                if (ret){
                    ...
                    System.out.println("点击进入签到  no 页面详情");
                }
            }
...
//        List<AccessibilityNodeInfo> list = nodeInfo.findAccessibilityNodeInfosByViewId("com.alibaba.android.rimet:id/home_bottom_tab_text");
        ...
        if (list.isEmpty()) {
            ...
            for (AccessibilityNodeInfo item : list) {
                String itemName = item.getText().toString();
                Log.v("xhw", "itemName " + itemName);
                if (item.equals(viewText)){
                    return onclick(item);  //遍历点击
                }
            }
        }
        return false;
